...
/*
  for (int i=0; i<=16; i++){
    ...
    if (CurrentLight < BrightestLight){
      BrightestLight = CurrentLight;
      BrightestCount1 = Counter - 1;
    }
  }

  AverageCount = ((BrightestCount + BrightestCount1)/2);
*/
  
  RightTurn90((turnForLDelay / 4) * BrightestCount.
  );
  StopRover();
  delay(1000);
  StartRover();
